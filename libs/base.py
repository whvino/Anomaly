...
class base(object):
    ...
    def _get_regularization_loss(self, optimized_parameters):
        ...
        regularization_loss = tf.add_n(regularization_loss) * self.config['weight_decay']   #tf.add_n Adds all input tensors element-wise.  weight decay权重衰减就是L2正则化
        #add_n的参数，若是一维列表，则将列表元素相加，得到标量，如[1,2,3]->6   若是二维，则对应元素相加 [[1,2,3],[4,5,6]] -> [5,7,9]
    ...
    #tf.summary模块主要用于tensorboard可视化
    def _get_summaries(self, losses, statistics):
        """
            define some summaries using tf.summary.scalar()
        """
        for name, loss in losses.items():
            tf.summary.scalar(name, loss)       #tf.summary.scalar()用来生成tensorboard图上一个节点
