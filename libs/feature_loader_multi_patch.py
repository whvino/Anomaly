    ...
    def __call__(self):
        ...
        for i in range(0, self.batch_size):
            if self.idx >= self.num_videos:
                random.shuffle(self.video_order)   #将序列所有元素随即排列（直接改变序列）
                self.idx = 0
...
class FeatureLoader(object):
    def __init__(self, hdf5_path, videos_txt, batch_size, clip_length):
        self.hdf5_path = hdf5_path  #train_feature_path
        self.videos_txt = videos_txt
        self.batch_size = batch_size

        self.clip_length = clip_length  #time_steps
        self.setup()
    ...
    def setup(self):
        ...
        for idx, video in enumerate(f_lines):  #enumerate()生成序号和原序列对象
            video = video.strip('\n')
            print(video)
            video_dict[video] = {}    #key:video  value:dict
            self.video_order.append(video)

            #dataset, video_name = video.split('.')[0].split('_train')
            feature_hdf5_path = os.path.join(self.hdf5_path, video)    #os.path.join(a,b) 拼接两个路径
